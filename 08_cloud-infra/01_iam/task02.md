...
- メリット
  - セキュリティが高い  
    明示的に許可されたリソースのみがアクセスできるため、不正アクセスを防ぐことができる。
  - リソースの可視性が高い  
    許可されたリソースのみがアクセス可能なため、どのリソースがアクセスされているかを把握しやすい。  
- デメリット
  - 管理が煩雑  
    リソースが増えると、許可するリソースを追加するため、管理が煩雑になる。  
  - 柔軟性が低い  
    事前に許可されたリソース以外のアクセスを許可しないため、柔軟性が低い。
- サービス例
  - PostgreSQLのRLS（Row Level Security）

### ブラックリスト方式

- メリット
  - 柔軟性が高い  
    事前に拒否されたリソース以外のアクセスを許可するため、柔軟性が高い。
  - 管理が容易  
    許可するリソースを追加する必要がないため、管理が容易。
- デメリット
  - 明示的な拒否が必要  
    事前に拒否されたリソース以外のアクセスを許可するため、拒否がない限りアクセス可能。  
  - リソースの可視性が低い  
    許可されたリソース以外のアクセスを許可するため、どのリソースがアクセスされているかを把握しにくい。  
- サービス例
  - Webフィルタリング
  - 電話の着信拒否
...
AWS Config  
AWS Configは、AWSリソースの設定の変更を監視し、  
設定が特定のルールに準拠しているかどうかを自動的にチェックすることもできる。  
